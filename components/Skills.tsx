...
const Skills: React.FC<Props> = ({ lang }) => {
  ...
  const skillSets = [
    ...
    {
      title: categories[2],
      icon: <Megaphone className="text-teal-500" size={32} />,
      items: lang === 'es'
    ...
    {
      title: categories[3],
      icon: <Cpu className="text-teal-500" size={32} />,
      items: lang === 'es'
        ? ['Git / GitHub', 'Metodologías Ágiles', 'Diseño de Experiencia e Interfaz', 'Planificación de Arquitectura']
        : ['Git / GitHub', 'Metodologias Ágeis', 'Design de Experiência e Interface', 'Planejamento de Arquitetura']
    }
  ];
  ...
  return (
    <section id="skills" className="py-32 bg-cool-dark">
      <div className="max-w-7xl mx-auto px-6">
        <h2 className="font-heading text-5xl md:text-7xl mb-20 text-center uppercase">{t.title1} <span className="text-slate-600">{t.title2}</span></h2>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {skillSets.map((set, idx) => (
            <div key={idx} className="p-10 bg-[#0a0a0c] border border-cool rounded-3xl hover:border-teal-500/50 transition-all duration-500">
