    ...
    <section id="experience" className="py-32 bg-slate-950/30">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
                ...
                <div className="relative z-10 text-center space-y-8 p-12">
                   <div>
                       <a 
                         ...
                         className="font-heading text-6xl md:text-8xl text-white mb-2 leading-none hover:text-teal-400 transition-colors block"
                       >
                         FOLKODE
                       </a>
                       <p className="text-teal-400 font-bold tracking-[0.4em] uppercase text-xs">Fundadora & Líder de Proyectos</p>
                   </div>
          ...
          <div>
            <span className="text-teal-500 font-bold tracking-[0.3em] uppercase text-xs">{t.tag}</span>
            ...
            <div className="relative pt-8 pb-12">
              ...
              <div className="relative ml-4 pl-10 border-l border-teal-500/20 space-y-12">
                {t.timeline.map((item: any, idx: number) => (
                  <div key={idx} className="relative group">
                    {/* Timeline dot */}
                    <div className="absolute -left-[49px] top-1.5 w-4 h-4 rounded-full border-2 border-teal-500 bg-[#0a0a0c] group-hover:bg-teal-500 transition-colors duration-500 shadow-[0_0_15px_rgba(20,184,166,0.3)]"></div>
                    
                    <div className="flex flex-col gap-1">
                      <span className="text-teal-500 font-heading text-xl mb-1">{item.year}</span>
                      <h4 className="text-white font-bold text-xl uppercase tracking-wide group-hover:text-teal-400 transition-colors">{item.title}</h4>
            ...
            <div className="mt-16 space-y-6">
              {t.cards && t.cards.map((card: any) => (
                <div key={card.num} className="flex items-start gap-6 p-6 bg-white/[0.01] border border-cool rounded-3xl hover:bg-white/[0.03] hover:border-teal-500/20 transition-all duration-300">
                  <div className="w-16 h-16 rounded-2xl bg-teal-500/10 flex items-center justify-center text-teal-400 font-heading text-2xl shrink-0">
                    {card.num}
                  </div>
                  <div>
